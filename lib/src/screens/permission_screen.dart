...
class _PermissionScreenState extends State<PermissionScreen> {
        ...
        children: [
          const Positioned.fill(
              child: ParticleBackground(
                  color: Colors.tealAccent, count: 20, opacity: 0.05)),
          SafeArea(
            child: Padding(
              padding:
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  const Text('We need a few permissions to keep you safe.',
                  ...
                  Expanded(
                    child: ListView(
                      children: [
                        _permCard('SMS', appState.smsGranted, Icons.message,
                            () async {
                        ...
                        _permCard('Contacts', appState.contactsGranted,
                            Icons.contacts, () async {
                          final res = await Permission.contacts.request();
                          appState.updatePermission
                              ?.call('contacts', res.isGranted);
                          if (res.isPermanentlyDenied) openAppSettings();
                        }, description: 'Access your emergency contacts'),
                        _permCard('Location', appState.locationGranted,
                            Icons.location_on, () async {
                          final res = await Permission.location.request();
                          appState.updatePermission
                              ?.call('location', res.isGranted);
                          if (res.isPermanentlyDenied) openAppSettings();
                        },
                            description:
                                'Share your live location in emergencies'),
                        _permCard('Microphone', appState.notificationsGranted,
                            Icons.mic, () async {
                          final res = await Permission.microphone.request();
                          appState.updatePermission
                              ?.call('microphone', res.isGranted);
                          if (res.isPermanentlyDenied) openAppSettings();
                        },
                            description: '⚠️ May record audio in background',
                            isWarning: true),
                        _permCard('Camera', appState.notificationsGranted,
                            Icons.camera_alt, () async {
                          final res = await Permission.camera.request();
                          appState.updatePermission
                              ?.call('camera', res.isGranted);
                          if (res.isPermanentlyDenied) openAppSettings();
                        },
                            description: '⚠️ May record video in background',
                            isWarning: true),
                        _permCard(
                            'Notifications',
                            appState.notificationsGranted,
                            Icons.notifications, () async {
                          final res = await Permission.notification.request();
                          appState.updatePermission
                              ?.call('notifications', res.isGranted);
                          if (res.isPermanentlyDenied) openAppSettings();
  ...
  Widget _permCard(
      String label, bool granted, IconData icon, VoidCallback onTap,
      {String description = '', bool isWarning = false}) {
    return InkWell(
      onTap: onTap,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: granted
                ? Colors.greenAccent.withAlpha((0.7 * 255).round())
                : (isWarning ? Colors.red.withAlpha(80) : Colors.white12),
            width: 1.2,
          ),
        ),
        child: Column(
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(granted ? Icons.check : icon,
                      color: granted
                          ? Colors.greenAccent
                          : (isWarning ? Colors.red : Colors.white70)),
                ),
